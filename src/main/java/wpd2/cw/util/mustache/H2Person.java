...
public class H2Person implements AutoCloseable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(H2Person.class);

    private Connection connection;

    public H2Person(ConnectionSupplier connectionSupplier) {
        try {
            connection = connectionSupplier.provide();
    ...
    public boolean isRegistered(String userName) {
        try (PreparedStatement ps = connection.prepareStatement("SELECT 1 FROM users WHERE name = ?")) {
    ...
    private boolean loginSQL(String userName, String password) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement("SELECT hash FROM users WHERE name = ?")) {
            ps.setString(1, userName);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    private boolean hasUserSQL(String userName) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement("SELECT name FROM users WHERE name = ?")) {
            ps.setString(1, userName);
            ResultSet rs = ps.executeQuery();
            return rs.next();
        }
    ...
    private static void execute(Connection connection, String cmd) {
        try {
